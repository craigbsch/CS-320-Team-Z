...
async def get_macronutrients(nutrient_type: str = Query(..., enum=["calories", "carbohydrates", "fat", "protein"], description="The type of macronutrient to fetch"),
                             current_user: dict = Depends(get_token_auth_header)):
    """
    Fetches summed values of a specified macronutrient from the database for an authenticated user, grouped by date.
    ...
    Returns:
        dict: A dictionary containing the user's ID, the requested nutrient type, and a list of dates and corresponding summed nutrient values.
    """
    user_id = current_user['uid']
    connection = get_db_connection()
    nutrient_data = []
    try:
        with connection.cursor() as cursor:
            sql = f"""
            SELECT date, SUM({nutrient_type}) as total_{nutrient_type}
            FROM {DATABASE_NUTRITION_TABLE}
            WHERE user_id = %s
            GROUP BY date
            ORDER BY date
            ...
            nutrient_data = [{
                "date": row['date'],
                nutrient_type: row[f"total_{nutrient_type}"]
